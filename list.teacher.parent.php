<?php
@session_start();
$activeTitle = "Teacher Parent Class list";
$activePage = "teacher.parent.class.list";
require 'up.html.php';
...
<?php require 'navbar.php'?>
  ...
  <h1 class='alert alert-primary mt-2'>Parent's Class List</h1>
  </div>
</div>
</div>
...
foreach ($haveClassIdArray as $index => $classId) {
    ...
    <tr>
        <th>{$classId}</th>
        <td><a href='list.parent.class.php?className={$className}' class='link-primary link-offset-2 link-underline-opacity-25 link-underline-opacity-100-hover'>{$className}</a></td>
    </tr>
    ";
}
