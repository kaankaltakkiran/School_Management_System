<?php
@session_start();
$activeTitle = "Teacher Give Note Class List";
$activePage = "teacher.give.note.class.list";
require 'up.html.php';
...
<?php require 'navbar.php'?>
  ...
  <h1 class='alert alert-primary mt-2'>Teacher Give Note Class List</h1>
  </div>
</div>
</div>
...
foreach ($haveClassIdArray as $index => $classId) {
    ...
    <tr>
        <th>{$classId}</th>
        <td><a href='give.note.php?className={$className}' class='link-primary link-offset-2 link-underline-opacity-25 link-underline-opacity-100-hover'>{$className}</a></td>
    </tr>
    ";
}
?>
